...
SELECT item FROM Orders Where amount >300;
...
SELECT Count(shipping_id) FROM Shippings WHERE status="Pending";

--5) Get how many customers have more than 1 item in "Pending" status
SELECT Count(customer) FROM Shippings WHERE status="Pending" AND customer >1;

--6) get all the customers that their country is USA and their age is higher than 25,
-- OR they are from UK and their age is lower than 25
SELECT first_name, last_name FROM Customers WHERE (country="USA" AND age >25) OR (country="UK" AND age <25);

--7) Get all the customers that are from UK and their first name starts with "D..."
SELECT first_name, last_name FROM Customers WHERE country="UK" AND first_name LIKE "D%";
